...
## 背景知识
  　　决策树算法也是十大经典算法之一，几乎每一本机器学习的书都会介绍决策树算法。1966年Hunt提出的CLS算法是最早的决策树算法。1986年Quinlan提出的ID3和1993年提出的C4.5是最具影响力的决策树算法。而今决策树算法已经成为了机器学习中最为经典的算法之一，受到了机器学习研究者的广泛关注。


## 算法介绍
### 决策树定义
　　决策树定义采用李航老师在《统计学习方法》一书中的定义：  
...
### 决策树学习
　　决策树的训练目标，就是训练出一种分类规则，使得数据集中所有的样本都能被分到其正确的分类。但这样的决策树可能有多个，也可能没有，所以我们的目标是找到一棵能将大部分样本正确分类并且具有较好的泛化能力的树。决策树学习的损失函数一般是正则化的极大似然函数。因为从所有决策树下选择最优的决策树是NP完全问题，因此一般会采用启发式算法近似求解。  
　　首先，先从根结点选择一个最优的特征对数据集进行划分，然后依次对其子结点进行最优的划分，即每一步求局部最优解，直至该子集所有样本都被正确地分类，则生成分类对应的叶子节点。如果子集还存在没有被正确分类的样本，则继续分裂，直至生成一棵完整的决策树。  
